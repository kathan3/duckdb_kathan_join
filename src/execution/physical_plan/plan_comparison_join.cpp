...
unique_ptr<PhysicalOperator> PhysicalPlanGenerator::PlanComparisonJoin(LogicalComparisonJoin &op) {
	...
	if (has_equality && !prefer_range_joins) {
		...
		auto EstimateKathanJoinCost = [&](idx_t left_card, idx_t right_card) {
			// KathanJoin: for testing, let's say it's very cheap
			return 0;
		};
		...
		return make_uniq<PhysicalKathanJoin>(op, std::move(left), std::move(right), std::move(op.conditions), op.join_type, op.estimated_cardinality);
		...
		// 	// Hash Join
		// 	auto hash_join = make_uniq<PhysicalHashJoin>(
		// 		op, std::move(left), std::move(right), std::move(op.conditions), op.join_type, op.left_projection_map,
		// 		op.right_projection_map, std::move(op.mark_types), op.estimated_cardinality, std::move(op.filter_pushdown));
		// 	hash_join->Cast<PhysicalHashJoin>().join_stats = std::move(op.join_stats);
		// 	return hash_join;
		// } else if (min_cost == ie_join_cost) {
		// 	// IE Join
		// 	return make_uniq<PhysicalIEJoin>(op, std::move(left), std::move(right), std::move(op.conditions), op.join_type,
